        ...
        <div className="data-list">
          {data.map((item, index) => (
            <div key={index} className="data-item">
              ...
              {/* Показываем nickname */}
              {item.publisher_nick && <p><strong>Автор:</strong> {item.publisher_nick}</p>}


              {/* Показываем количество фотографий */}
              {item.files && item.files.length > 0 && (
                <p><strong>Количество фотографий:</strong> {item.files.length}</p>
              )}  

              {/* Показываем изображение, если оно есть */}
              {item.imageUrl && (
                <img src={item.imageUrl} alt={item.title} className="data-image" />
              )}
            </div>
          ))}
        </div>
